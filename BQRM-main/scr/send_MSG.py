...
def main():
    ...
    BQRM_DOCS_FOLDER_NAME = f'../bulletins/{AA_today}/{MM_today}/{DD_today}' # Adjust this path if your BQRM folder is elsewhere
    ...
    logger.debug(f"Does Path.exists() find the file? {bqrm_docx_file.exists()}")
    logger.debug(f"Does os.path.exists() find the file? {os.path.exists(bqrm_docx_file)}")
    logger.debug(f"Resolved path: {bqrm_docx_file.resolve()}")
    

# Add a small sleep here to allow any external process to finish writing the file
    import time
    logger.debug("Sleeping for 5 seconds to allow file creation to complete (if timing issue).")
    time.sleep(5) # Wait for 5 seconds

    logger.debug(f"Does Path.exists() find the file after sleep? {bqrm_docx_file.exists()}")
    logger.debug(f"Does os.path.exists() find the file after sleep? {os.path.exists(bqrm_docx_file)}")
# -----------------------------
